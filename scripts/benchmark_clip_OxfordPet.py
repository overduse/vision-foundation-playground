...
# Note: Pets images are larger than CIFAR.
# If you encounter OOM (Out Of Memory) errors, reduce BATCH_SIZE to 32 or 16.
BATCH_SIZE = 64
SEED = 42
...
# --- Prompt Templates (Tailored for Pets) ---
PROMPT_TEMPLATES = {
    "Raw Label": lambda c: f"{c}",
    "Standard": lambda c: f"a photo of a {c}",
    "Pet Context": lambda c: f"a photo of a {c}, a type of pet",
    "Close-up": lambda c: f"a close-up photo of a {c}",
}
...
def setup_logger(model_name):
    ...
    Saves logs to both file and console.
    """
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    log_filename = f"run_pets_{model_name}_{timestamp}.log"
    log_path = os.path.join(LOG_DIR, log_filename)

    logger = logging.getLogger(f"Benchmark-Pets-{model_name}")
...
class BenchmarkTransform:
    ...
    def __call__(self, image):
        # process image and return the tensor
...
def run_single_model_benchmark(model_name):
    """
    Runs the benchmark for a single CLIP model on the Oxford-IIIT Pet dataset.
    """
    ...
    # Prepare Dataset
    logger.info("[INFO] Downloading/Loading Oxford-IIIT Pet dataset...")
    
    transform_pipeline = BenchmarkTransform(processor)
    
    # download=True will download the dataset (~800MB) if not present
    try:
    ...
    # Clean class names (replace underscores with spaces for better semantic understanding)
    # Example: 'havanese_dog' -> 'havanese dog'
...
if __name__ == "__main__":
    ...
    print(f"\n[GLOBAL] STARTING PETS BENCHMARK ON: {MODELS_TO_TEST}\n")
    ...
    summary_filename = f"summary_pets_{session_timestamp}.txt"
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        log_both(f, "="*80)
        log_both(f, f"GRAND SUMMARY REPORT - OXFORD PETS - {session_timestamp}")
        log_both(f, "="*80)
